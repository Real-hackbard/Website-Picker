...
function MyGetFileSize(const Filename: string):ULarge_Integer;// TULargeInteger;
var
  Find: THandle;
  Data: TWin32FindData;
begin 
...
begin
  ...
  begin
    MessageBoxA( Handle, PansiChar('The target folder does not exist.!'),
                         PansiChar('Destination Folder'), MB_ICONINFORMATION );
    Exit;
  end;
  ...
  begin
    MessageBoxA( Handle, PansiChar('Enter a URL !'), PansiChar('URL ?'), MB_ICONINFORMATION );
    Exit;
  end;
        ...
        if WebPicker.GetLiens.Count <= 0 then
          ...
          begin
           ...
           //     DownFile.Connection( WebPicker.Getliens.Strings[X] );
                DownFile.GetFileToLocalFile({ExtractFileNameToURL}( WebPicker.Getliens.Strings[X] )  ,Slach(Edit2.Text));
                DownFile.Free;
              except
                DownFile.Free;
                WebPicker.Free;
